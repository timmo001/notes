...
The daemon uses a separate loopback-only OpenCode 2 server on port 4097. Its base configuration and `notes-daemon` agent live under `.opencode-daemon/` in this repository rather than the interactive global OpenCode configuration. The service generates an isolated runtime configuration with the private `allowedReadPaths` and separate XDG config, data, state, and cache directories. The server runs in pure mode with external skills, project config, and default plugins disabled. Only the read-only GitHub MCP endpoint and Notes MCP server are configured. Built-in web search is enabled and pinned to Exa for every configured model.
